...
Search algorithm are of great importance in many domains of mathematics and computer science. The search problem that will be discussed in the following sections can be formulated in simple terms as follows:
...
Using this definition of the search problem, the goal of a search algorithm is to find all states $t\in\begin{mathcal}S\end{mathcal}$ for which $\Or(s) = 1$. For the sake of simplicity, we will assume in the following sections that the solution set $\begin{mathcal}T\end{mathcal}$ contains only one single state $t$.

\smallskip

To map the search problem above to a form suitable for solution by a digital (quantum) computer, we can also number and encode the $N$ input states $s_i \in \begin{mathcal}S\end{mathcal}$ in binary form such that $s_i = \sum\limits_{j=0}^l s_{ij}2^j $, where $l$ is the minimum required length of a binary register that can hold all $N$ input states. With this definition, it is then also trivial to reformulate $\Or$ such that the function operates on a binary input register instead of the original states. 

\smallskip

Using these assumptions and definitions, it can then be shown that the most efficient classical search algorithm for solving the search problem above will use $\begin{mathcal}O\end{mathcal}(N)$ calls of the function $\Or$ to find the solution $t$ to the search problem. Assuming that the time to evaluate the function $\Or$ is far superior to the time needed to perform any other operation during the search algorithm, the number of calls to $\Or$ corresponds approximately to the runtime of the whole search algorithm.

\smallskip

Amazingly, in 1997, \cite{Grover_Quantum_1997} found a quantum algorithm that could solve the same search problem with only $\begin{mathcal}O\end{mathcal}(\sqrt{N})$ calls to the function $\Or$. His algorithm achieves this by repeatedly calling a quantum-mechanical implementation of the function $\Or$ with a highly superposed qubit state and applying a special operator to the output state afterwards. The individual steps of the algorithm are as follows:

\begin{enumerate}
 \item Start with a qubit register in the state $\ket{\psi} = \ket{000\hdots 0}$.
 ...
 \subitem a) Apply the so-called {\it Oracle operator} $\ket{i}\to (-1)^{\Or(i)}\ket{i}$ to the state $\ket{\psi}$.
 \subitem b) Apply the so-called {\it diffusion operator} $\ket{i} \to -\ket{i}+\frac{2}{N}\sum\limits_i^N\ket{j}$ to the state $\ket{\psi}$.
	\item Measure the state of the quantum register
\end{enumerate}

As can be seen, the algorithm makes use of quantum parallelism to solve the search problem faster than any classically conceivable algorithm. The different steps of the algorithm can be explained in the following, more intuitive way:
...
\item Then, the algorithm applies the so-called Oracle operator to this superposed state. The effect of the Oracle is that it flips the sign of the basis state that corresponds to the solution of the search problem. As will be shown later, such an Oracle operator can be easily implemented for any possible classical search function.
\item In the next step, the diffusion operator transfers a fraction of the amplitude of those states with positive signs to the state with negative sign, giving that state thereby a higher overall amplitude compared to the other states and also flipping back its sign to positive.
\item By repeating these two operations, the amplitude of the state which corresponds to the solution of the search problem will increase in a mononous way until the amplitudes of all the other states are zero.
\end{enumerate}

A rather interesting and intuitive derivation of the algorithm starting from Schrödinger's equation has been given in a seminal paper by \cite{grover_schrodingers_2001} and shall be briefly rediscussed here. The derivation begins by considering a quantum system governed by Schrödinger's equation, which can be written as (omitting all physical constants for the sake of clarity)
...
It is also possible to define a version of the Grover algorithm where the Oracle function does not encode the marked state directly in the input qubit register but uses an ancialla qubit to store the information on the marked state. Possible implementations of such ancilla-based Quantum oracle functions for the two-qubit case are shown in fig. \ref{fig:GroverOracleImplementations}. There, a two-qubit Tofolli gate in combination with several single-qubit NOT gates (which can be easily implemented as single-qubit $X_{\pi}$ rotations) is used to flip the state of an ancilla-qubit conditionally on the input state of the gate. The Toffoli gate is fully reversible and can operate on product- and superposition states. Representing the quantum Oracle in this form is quite useful since it allow us to make a direct comparision of the Grover algorithm to a classical search algorithm implemented using reversible logic gates. This will in turn allow us to measure the fidelity and success probability of our algorithm againsts that of a classical search algorithm and thereby quantify the quantum speed-up achieved with our system. 
...
Fig. \ref{fig:GroverAlgorithmFullSchematic} shows such an ancilla-based implementation of the two-qubit Grover algorithm. This algorithm proceeds almost identically as the compiled version which we will present in the next section, with the difference that it uses a 3-qubit control-not (CNOT) gate $C$ of the form

\begin{equation}
C = \mathrm{I}^{n\otimes n}-2\sum\limits_{ij} \ket{ij1}\bra{ij1}
\end{equation}

which flips the sign of all the states for which the ancilla qubit is in state $\ket{1}$ and hence converts the ancilla-based quantum Oracle into the phase-encoded version that was outlined before. Here, the ancilla qubit must not be read out before the two input qubits at the end of the algorithm since it will destroy the prepared state of the first two qubits otherwise. Reading out the value of the ancilla qubit after having read out the input qubits allows one to check that there exists indeed a solution to the search problem, in which case the ancilla qubit should have a value of $\ket{1}$.
...
\begin{figure}
	\centering
		\includegraphics[width=1.35\textwidth]{"./data/ct5/2011_04_21 - grover and tomo/good_data/grover algorithm - single run results"}
