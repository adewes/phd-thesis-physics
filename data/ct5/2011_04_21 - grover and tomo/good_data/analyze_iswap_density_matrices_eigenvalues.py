...
for i in range(0,len(spinMatrix)):
	...
	initialMatrix/=4
	initialMatrixPhysical = renderMatrixPhysical(initialMatrix)

	print eigh(initialMatrix)[0]
	print abs(trace(initialMatrix))

	import scipy.optimize
	
	result = scipy.optimize.fmin(probability,parametrizeMatrix(initialMatrixPhysical),args = (spinMatrix,row),maxiter = 1000,maxfun = 100000)
