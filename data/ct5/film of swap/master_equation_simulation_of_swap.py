...
data = Datacube()
data.loadtxt("State Tomography of Swap vs Swap Duration_Quantum Process Tomography at time 23-2_0_Swap of state 0_0_180_0-0_0_Measured spins-300_0")
...
g = -0.0094*math.pi*2.0

times = arange(0,200,1)

Delta = 0.00*math.pi*2.0
Delta1 = -0.001*math.pi*2.0
Delta2 = 0.001*math.pi*2.0

#states = [tensor(gs,es)*0.9+0.1*tensor(es,es)]
...
for m in measurements:
	initialMatrix+=data[m][row]*tensor(matrixMap[m[0]],matrixMap[m[1]])

initialMatrix = renderMatrixPhysical(initialMatrix)

states = [initialMatrix]
...
offset = 0.0
for measurement in measurements:
	plot(data["t"],data[measurement]+offset,cube["t"],cube[measurement]+offset)
	offset+=2.0
show()
